...
/* 531 · Mobil tablo başlıklarını masaüstü THEAD üzerinden üretir. */
(function(){
  'use strict';
  ...
  function cleanHeaderText(cell){
    ...
    clone.querySelectorAll('button,input,select,svg,.sort-icon,.tcr-sort-icon').forEach(el=>el.remove());
    return (clone.textContent||'').replace(/\s+/g,' ').trim();
  }

  function applyMobileLabels(table){
    if(!(table instanceof HTMLTableElement)) return;
    const headers=Array.from(table.tHead?.rows?.[0]?.cells||[]).map(cleanHeaderText);
    ...
    Array.from(table.tBodies||[]).forEach(tbody=>{
      Array.from(tbody.rows||[]).forEach(row=>{
        ...
        Array.from(row.cells||[]).forEach(cell=>{
          ...
          if(/^(işlem|işlemler|aksiyon|aksiyonlar)$/i.test(cell.getAttribute('data-label')||'')){
            cell.classList.add('tcr-action-column');
          }
          columnIndex+=span;
        });
    ...
    table.dataset.tcrMobileLabels='ready';
  }

  function scan(root){
    const scope=root&&root.querySelectorAll?root:document;
    if(scope instanceof HTMLTableElement) applyMobileLabels(scope);
    scope.querySelectorAll?.('table.data,table.tcr-mobile-cards').forEach(applyMobileLabels);
  }
...
/* 530 — Global automatic hint rule.
   Adds a hint only when visible text is actually shortened by overflow/ellipsis.
   KPI titles/descriptions/badges are included; the rule also works for other
   explicitly ellipsized text throughout the application. */
(function(){
  'use strict';
  ...
  var SELECTOR = [
    ...
    '.kpi-value',
    '.kpi-sub',
    '.kpi-card [class*="title"]',
    '.kpi-card [class*="value"]',
    '.erp-status-kpi [class*="title"]',
    '.erp-status-kpi [class*="value"]',
    '[data-auto-hint]'
  ].join(',');

  function textOf(el){
    return String(el.getAttribute('data-hint-text') || el.textContent || '')
      .replace(/\s+/g,' ')
      .trim();
  }
  ...
  function isTruncated(el){
    ...
    var clips = style.overflow === 'hidden' || style.overflowX === 'hidden' ||
  ...
  function update(el){
    ...
    var value = textOf(el);
    var shortened = value && isTruncated(el);

    if(shortened){
      if(!el.hasAttribute('data-tcr-original-title')){
        el.setAttribute('data-tcr-original-title', el.getAttribute('title') || '');
      }
      el.setAttribute('title', value);
      el.setAttribute('data-tcr-auto-hint', 'true');
      if(!el.hasAttribute('aria-label')) el.setAttribute('aria-label', value);
    }else if(el.getAttribute('data-tcr-auto-hint') === 'true'){
      var original = el.getAttribute('data-tcr-original-title') || '';
      if(original) el.setAttribute('title', original); else el.removeAttribute('title');
      el.removeAttribute('data-tcr-auto-hint');
      el.removeAttribute('data-tcr-original-title');
  ...
  function scan(root){
    ...
    if(scope.matches && scope.matches(SELECTOR)) update(scope);
  ...
  function boot(){
    ...
    document.addEventListener('tcr:page-rendered', function(){ schedule(document); });
...
/* 529 · Mobil tablolarda kayıt anahtarı ilk hücre, durum ikinci hücre. */
(function(){
  'use strict';

  const primaryPattern = /^(kod|cari kod|stok kodu|stok kod|işlem no|islem no|hareket no|fiş no|fis no|belge no|evrak no|sipariş no|siparis no|teklif no|fatura no|çek no|cek no|senet no|kayıt no|kayit no)$/i;
  const statusPattern = /^(durum|statü|statu)$/i;
  ...
  function markTable(table){
    ...
    Array.from(table.tBodies || []).forEach(tbody => {
      Array.from(tbody.rows || []).forEach(row => {
        ...
        cells.forEach(cell => {
          cell.classList.remove('tcr-primary-id-cell','tcr-status-cell');
          const label = normalize(cell.getAttribute('data-label'));
          const col = normalize(cell.getAttribute('data-col'));
          if(primaryPattern.test(label) || primaryPattern.test(col)) cell.classList.add('tcr-primary-id-cell');
          if(statusPattern.test(label) || statusPattern.test(col)) cell.classList.add('tcr-status-cell');
        });

        if(!row.querySelector('.tcr-primary-id-cell')){
          const fallback = cells.find(cell => !cell.classList.contains('tcr-action-column'));
          if(fallback) fallback.classList.add('tcr-primary-id-cell');
        }
      });
    });
  }
  ...
  function scan(root){
    const scope = root && root.querySelectorAll ? root : document;
    if(scope instanceof HTMLTableElement) markTable(scope);
    scope.querySelectorAll?.('table.data, table.tcr-mobile-cards').forEach(markTable);
  }
